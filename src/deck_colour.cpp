...
bool parse_hex(std::string_view const& value, Colour& target)
{
	if (value.size() == 3)
	{
		target.r = hex_to_nibble(value[0]) << 8;
		target.g = hex_to_nibble(value[1]) << 8;
		target.b = hex_to_nibble(value[2]) << 8;
		target.a = 255;
		return true;
	}
...
bool parse_colour(std::string_view const& value, Colour& target)
{
	if (value.size() == 0)
		return false;
...
DeckColour::DeckColour(std::string_view const& value)
{
	if (!parse_colour(value, m_colour))
		m_colour.set_pink();
}
